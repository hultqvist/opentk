    ...
    public class GLWidget : DrawingArea, IDisposable
    {
        ...
        void InitializegraphicContext()
        {
            ...
            GraphicsMode graphicsMode = new GraphicsMode(
                                        colorBufferColorFormat,
                                        DepthBPP,
                                        StencilBPP,
                                        Samples,
                                        accumulationColorFormat,
                                        buffers,
                                        Stereo
            );
            ...
            } else if (Configuration.RunningOnMacOS)
            {
                ...
                bool ownHandle = true;
                bool isControl = true;
                WindowInfo = OpenTK.Platform.Utilities.CreateMacOSCarbonWindowInfo(
                                                windowHandle,
                                                ownHandle,
                                                isControl
                );
            } else if (Configuration.RunningOnX11)
            {
                ...
                WindowInfo = OpenTK.Platform.Utilities.CreateX11WindowInfo(
                                                display,
                                                screen,
                                                windowHandle,
                                                rootWindow,
        ...
        {
            return XGetVisualInfoInternal(
                                display,
                                (IntPtr)(int)vinfo_mask,
                                ref template,
                                out nitems
            );
        }
